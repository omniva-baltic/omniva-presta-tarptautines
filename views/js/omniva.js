...
(function ( $ ) {
    $.fn.omniva = function(options) {
        ...
        function calculateDistance(y,x){
            ...
            $.each( terminals, function( key, location ) {
                distance = calcCrow(y, x, location['y_cord'], location['x_cord']);
        ...
        function initMap(){
          ...
          var locations = omnivaTerminals;
            jQuery.each( locations, function( key, location ) {
              L.marker([location['y_cord'], location['x_cord']], {icon: terminalIcon, terminalId:location['id'] }).on('click',function(e){ listTerminals(locations,0,this.options.terminalId);terminalDetails(this.options.terminalId);}).addTo(map);
            });
    ...
    init : function() {
        ...
        $('select[name="omnivalt_parcel_terminal"]').off('change.Omniva').on('change.Omniva', function(e) {
            ...
            $.ajax({
                ...
                url: omniva_front_controller_url,
                cache: false,
                dataType: 'json',
                data: 'action=saveTerminal' + '&terminal=' + terminal,
