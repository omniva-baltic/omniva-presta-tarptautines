...
class OmnivaIntBulkActionController extends FrameworkBundleAdminController
{
    ...
    public function bulkGenerateLabels(Request $request)
    {
        ...
        foreach($orders as $id_order)
        {
            ...
            if($omnivaOrder && \Validate::isLoadedObject($omnivaOrder) && \Validate::isLoadedObject($order) && \Validate::isLoadedObject($cart))
            {
                ...
                if(!$order)
                {
                    $this->flashErrors(\Translate::getModuleTranslation($module_legacy, 'Could not build request for order %s.', $module_legacy->name, [$id_order]));
                    continue;
                }
                ...
                catch (OmnivaApiException $e)
                {
                    ...
                    $this->flashErrors(\Translate::getModuleTranslation($module_legacy, $message, $module_legacy->displayName));
                    continue;
                }
                ...
                if($response && isset($response->shipment_id, $response->cart_id))
                {
                    ...
                    {
                        $this->flashErrors(\Translate::getModuleTranslation($module_legacy, 'Couldn\'t update Omniva order: #%s.', $module_legacy->name, [$id_order]));
                    }
                    else
                    {
                ...
                {
                    $this->flashErrors(\Translate::getModuleTranslation($module_legacy, 'Failed to receive a response from API. Order: #%s.', $module_legacy->name, [$id_order]));
                    return false;
                }
            }
        }
    ...
    public function bulkPrintLabels(Request $request)
    {
        ...
        foreach($orders as $id_order)
        {
            ...
            {
                $this->flashErrors(\Translate::getModuleTranslation($module_legacy, 'Failed to get labels from API for order #%s. Please try again later.', $module_legacy->name, [$id_order]));
            }
        }
